

local BASEDATA = {
    RAINBOW_KEY_ACTIVE = 0,

    DRILLDATA = {
        IS_ACTIVE = 0,
        ITEM_ID = 0,
        FORCE_SET_AMBUSH = 0,
        IGNORE_RESET_DATA = 0,
    },
    DRILL_PREIDX = 83,

    GOOD_JUICE_NUM = 0,

    PREFERRED_OPTIONS_LASTITEM = CollectibleType.COLLECTIBLE_BREAKFAST,
    PREFERRED_OPTIONS_LASTPRICE = 0,
    PREFERRED_OPTIONS_TOOKITEM = 0,
    PREFERRED_OPTIONS_MARKROOMFORCLEAR = -1,

    CUSTOM_PILL_POOL = 0,
    PILLS_TOTAL = 0,
    PILLS_GOOD = 0,
    PILLS_NEUTRAL = 0,
    PILLS_BAD = 0,
    PILL_COLORS = 0,
    PILL_TABLES_CALCULATED = 0,
    PILLS_BAD_NUM = 0,
    PILLS_GOOD_NUM = 0,
    PILLS_NEUTRAL_NUM = 0,

    MILCOM_COINS = 0,
    MILCOM_KEYS = 0,
    MILCOM_BOMBS = 0,

    BIGBLIND_INDEXES = {[0]=0},
    BIGBLIND_FINALSTR = {[0]=0},
    BIGBLIND_CONBOI_ACTIVE = 0,

    QUAKEBOMBS_GRIDS = {},

    FOILCARD_VALUE = 0,
    MANTLEDARK_USES = 0,

    TINTED_ROOM_POSITIONS = {},

    ZAP_CAP_COUNTER = 0,

    TRENDSETTER_ACTIVE = false,
    TRENDSETTER_LIST = {},

    LIBRARY_CARD_VISITED_IDXS = {},

    GOLDEN_KEY_STATE = false,
    GOLDEN_BOMB_STATE = false,
    THEGATE_STATE = 0,

    TEMPLE_TRIAL_ROOMS = {},
    TEMPLE_MAIN_ROOMS = {},

    BLOOD_MONEY_COUNTERS = 0,

    CABAN_ITEMINDEXES = {},
    CABAN_COLLECTED_NUM = 0,

    MULAC_FULLCLEARED = false,

    EB_GRASS_DONATION_COUNTER = 0,

    IX_REVEALED_ROOM = -1,
    IX_FAILED = 0,

    AHAU_TRIGGERED = 0,
    AHAU_ROOM_SUBS = {},
    AHAU_ROOM_UPGR = {},

    CHUEN_MONKEY_TOREMOVE = 0,
    CHUEN_MONKEY_REMOVED_PERROOM = {},

    MINDS_EYE_POSITIONS = {},

    MONOCLE_SPAWNS = {
        {5,20,1,0},
    },

    XENOTRANSFUSION_DAMAGE_COUNTER = {},

    SAVED_PEDESTAL_TRINKETS = {},

    LADYBUG_SPAWNS = {},
    LADYBUG_TRIGGERED = false,

    BROTHERHOOD_ENTERED_ANGEL = false,

    TMTAMER_IDS = {},

    SUPER_RETROFALL_ID = 3,
    MATTMAN_CHANCE_LAMB_KILLS = 0,

    EQUALIZER_PICKUP = "",
    EQUALIZER_STAT = "",
    EQUALIZER_COINS = 0,
    EQUALIZER_KEYS = 0,
    EQUALIZER_BOMBS = 0,
}

return BASEDATA